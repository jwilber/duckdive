...
column_colors = [
    "cyan", "green", "yellow", "magenta", "red", "white", "bright_green", "bright_blue", "bright_magenta"
]
...
def format_ratings_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df['rating_key'] = df['rating'].apply(lambda x: x.get('key') if isinstance(x, dict) else None)
    df['rating_value'] = df['rating'].apply(lambda x: x.get('value') if isinstance(x, dict) else None)
...
def format_swells_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - Flattening the nested 'swells' column into multiple rows and columns
    
    :param df: The DataFrame containing swells data
    :return: The flattened DataFrame with individual swell data
    """
    swell_rows = []
    for idx, row in df.iterrows():
        swells = row['swells']
        if isinstance(swells, list):
            for swell in swells:
                swell_row = row.drop('swells').to_dict()  # Keep other row data
                swell_row.update(swell)  # Add swell data to row
                swell_rows.append(swell_row)
    
    return pd.DataFrame(swell_rows)
...
def format_conditions_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['forecaster_name'] = df['forecaster'].apply(lambda x: x.get('name') if isinstance(x, dict) else None)
    df['forecaster_avatar'] = df['forecaster'].apply(lambda x: x.get('avatar') if isinstance(x, dict) else None)
    df = df.drop(columns=['forecaster'])  # Drop the original "forecaster" column
    
    # Destructure 'am' into separate columns
    df['am_timestamp'] = df['am'].apply(lambda x: x.get('timestamp') if isinstance(x, dict) else None)
    df['am_observation'] = df['am'].apply(lambda x: x.get('observation') if isinstance(x, dict) else None)
    df['am_rating'] = df['am'].apply(lambda x: x.get('rating') if isinstance(x, dict) else None)
    df['am_minHeight'] = df['am'].apply(lambda x: x.get('minHeight') if isinstance(x, dict) else None)
    df['am_maxHeight'] = df['am'].apply(lambda x: x.get('maxHeight') if isinstance(x, dict) else None)
    df['am_plus'] = df['am'].apply(lambda x: x.get('plus') if isinstance(x, dict) else None)
    df['am_humanRelation'] = df['am'].apply(lambda x: x.get('humanRelation') if isinstance(x, dict) else None)
    df['am_occasionalHeight'] = df['am'].apply(lambda x: x.get('occasionalHeight') if isinstance(x, dict) else None)
    df = df.drop(columns=['am'])  # Drop the original "am" column

    # Destructure 'pm' into separate columns
    df['pm_timestamp'] = df['pm'].apply(lambda x: x.get('timestamp') if isinstance(x, dict) else None)
    df['pm_observation'] = df['pm'].apply(lambda x: x.get('observation') if isinstance(x, dict) else None)
    df['pm_rating'] = df['pm'].apply(lambda x: x.get('rating') if isinstance(x, dict) else None)
    df['pm_minHeight'] = df['pm'].apply(lambda x: x.get('minHeight') if isinstance(x, dict) else None)
    df['pm_maxHeight'] = df['pm'].apply(lambda x: x.get('maxHeight') if isinstance(x, dict) else None)
    df['pm_plus'] = df['pm'].apply(lambda x: x.get('plus') if isinstance(x, dict) else None)
    df['pm_humanRelation'] = df['pm'].apply(lambda x: x.get('humanRelation') if isinstance(x, dict) else None)
    df['pm_occasionalHeight'] = df['pm'].apply(lambda x: x.get('occasionalHeight') if isinstance(x, dict) else None)
...
def format_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - Flattening the nested "swells" column if it exists
    """
    ...
    for column, unit in units_mapping.items():
        if column in df.columns:
            df[column] = df[column].apply(lambda x: f"{x} {unit}" if pd.notnull(x) else x)
