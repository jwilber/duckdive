...
def construct_surfline_api_url(
    ...
    days=None,  # Default to None so it's not added if not set
    interval_hours=None,  # Default to None so it's not added if not set
    max_heights=None,  # Default to None so it's not added if not set
    sds=None,  # Default to None so it's not added if not set
    access_token=None,
    forecast_type="wave"  # Default to 'wave' type
):
    """
    ...
    :param forecast_type: String, type of data to include (must be 'rating', 'conditions', 'swells', 'sunlight', 'wave', 'wind', 'tides', 'sunlight', 'swells', or 'weather')
    :return: String, constructed API URL
    """
    base_url = "https://services.surfline.com/kbyg/spots/forecasts"

    # Validate forecast_type
    valid_types = ['rating', 'conditions', 'swells', 'sunlight', 'wave', 'wind', 'tides', 'sunlight', 'swells', 'weather']
    if forecast_type not in valid_types:
        raise ValueError(f"Invalid forecast_type. Must be one of {valid_types}")

    # Construct basic query with only spot_id
    params = {'spotId': spot_id}

    # Add optional parameters if provided
    if days is not None:
        if days < 1 or days > (17 if access_token else 6):
    ...
    # Construct the URL with the appropriate forecast type
